...
def create_item(price: int):
    ...
    try:
        db.set(key, value)
        publish_event('stock_events', {'event': 'item_created', 'item_id': key, 'price': price})
    except redis.exceptions.RedisError:
        return abort(400, DB_ERROR_STR)
...
def add_stock(item_id: str, amount: int):
    ...
    try:
        ...
        publish_event('stock_events', {'event': 'stock_added', 'item_id': item_id, 'amount': amount})
    except redis.exceptions.RedisError:
        return abort(400, DB_ERROR_STR)
        ...
        with db.pipeline() as pipe:
            while True:
                try:
                    ...
                    pipe.publish('stock_events', msgpack.encode({'event': 'stock_subtracted', 'item_id': item_id, 'amount': amount}))
                    pipe.execute()
                    break
