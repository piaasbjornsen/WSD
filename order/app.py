...
def create_order(user_id: str):
    ...
    try:
        db.set(key, value)
        publish_event('order_events', {'event': 'order_created', 'order_id': key, 'user_id': user_id})
    except redis.exceptions.RedisError:
        return abort(400, DB_ERROR_STR)
...
def add_item(order_id: str, item_id: str, quantity: int):
    ...
    try:
        ...
        publish_event('order_events', {'event': 'item_added', 'order_id': order_id, 'item_id': item_id, 'quantity': quantity})
    except redis.exceptions.RedisError:
        return abort(400, DB_ERROR_STR)
...
def checkout(order_id: str):
    ...
    for item_id, quantity in items_quantities.items():
        stock_reply = retry_post_request(f"{GATEWAY_URL}/stock/subtract/{item_id}/{quantity}")
        if not stock_reply or stock_reply.status_code != 200:
            rollback_stock(removed_items)
            abort(400, f'Out of stock on item_id: {item_id}')
        removed_items.append((item_id, quantity))
    user_reply = retry_post_request(f"{GATEWAY_URL}/payment/pay/{order_entry.user_id}/{order_entry.total_cost}")
    ...
    try:
        db.set(order_id, msgpack.encode(order_entry))
        publish_event('order_events', {'event': 'payment_completed', 'order_id': order_id})
    except redis.exceptions.RedisError:
        return abort(400, DB_ERROR_STR)
